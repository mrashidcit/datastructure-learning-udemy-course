...
public class AVL {
    ...
    private BinaryNode insertNode(BinaryNode node, int nodeValue) {
        ...
        if (balance > 1 && nodeValue > node.left.value) { // LR Condition
            node.left = rotateLeft(node.left);
            return rotateRight(node);
        }

        if (balance < -1 && nodeValue > node.right.value) { // RR Condition
            return rotateLeft(node);
        }

        if (balance < -1 && nodeValue < node.right.value) { // RL Condition
